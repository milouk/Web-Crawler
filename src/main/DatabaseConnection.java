...
/**
 * <h1>DataBase Connection Class - DatabaseConnection.java</h1> This class
 * consists exclusively of static methods that operate on a database. It
 * contains methods that connect to the database, disconnect from it and update
 * it.
 *
 * @author Complet
 * @version 5.0
...
public class DatabaseConnection {
	 ...
	 *
	 * @throws Exception
	 *             if the registration to the JDBC driver fails.
	 * @throws SQLException
	...
	public static void openDatabaseConnection() {
		try {
			GetCredentials.dbCredentials("path DataBase-Credentials.txt"); // Calls
																			...
																			// dbCredentials
																			// method
			Class.forName("com.microsoft.sqlserver.jdbc.SQLServerDriver"); // Registers
																			// to
																			// the
																			// JDBC
																			// driver
			...
			stmt = dbcon.createStatement(); // Creates a statement that
	...
	/**
	 * Inserts data into the database. The array argument specifies an ArrayList
	 * that contains URLs. The path argument is a specifier for a file that
	 * contains HTML text.
	 *
	 * @array an ArrayList that contains the URLs to be inserted in the database
	 *        in form of Strings.
	 * @param path
	 *            a path that points to the file where the HTML text of the
	 *            links is saved.
	 * @throws SQLException
	 *             if an error occurs during the data insertion.
	 */
	...
	public static void InsertData(String links, String path) {
		try {
			...
			pathHtml = path + "/" + Integer.toString(Mainclass.getPosition()) + ".html"; // Specifies
																							// the
																							// path
																							// of
																							// each
	...
	/**
	 * Deletes all the columns from a database table. The tbName specifies a
	 * table's name which already exists in the database.
	 *
	 * @tbName the name of a table in the database which columns will be deleted
	 * @throws SQLException
	 *             if an error occurs during the data deletion
	 */
